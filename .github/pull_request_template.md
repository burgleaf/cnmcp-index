...
- [ ] 已提供 `README.md`，说明资源解决的问题、核心能力、适用人群、使用注意事项和官方资源。
- [ ] 字段符合 `schemas/resource.schema.json`，标签来自 `catalog/tags.json`。
- [ ] 已提供公开 HTTPS 源码地址、许可证、作者，以及职业/任务导向标签。
- [ ] 如为 Plugin，已确认它为 AI 工具增加功能、工作流或界面能力；不限制为编程工具插件。
- [ ] 如填写平台支持，只依据上游声明，包含核验日期并尽量提供 `evidenceUrl`；没有明确声明可省略该字段。
- [ ] 详情页由站点按 AI 工具生成简明安装内容，第三方命令不会由站点或审核流程自动执行。
