...
function maintenanceStatus(pushedAt, fetchedAt, archived) {
  if (archived) return "archived";
  if (!pushedAt || !fetchedAt) return "low-activity";
  const pushed = Date.parse(pushedAt);
  const fetched = Date.parse(`${fetchedAt}T23:59:59Z`);
  if (!Number.isFinite(pushed) || !Number.isFinite(fetched) || pushed > fetched) return "low-activity";
  return (fetched - pushed) / DAY_MS <= 180 ? "active" : "low-activity";
}
...
export function computeResourceQualityCore(input) {
  ...
  return Object.freeze({
    ...
    maintenanceStatus: maintenanceStatus(input.pushedAt, input.dataUpdatedAt ?? input.fetchedAt, input.archived),
